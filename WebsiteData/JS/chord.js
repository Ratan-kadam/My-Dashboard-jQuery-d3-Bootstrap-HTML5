...
function generateStationMap(city) {
    city = city || "San Jose";
    stationList = [];
    $.getJSON("http://52.39.148.231:3000/getData?filename=station", function (data) {
...
function getTrips(city) {
    tripsData = []
    if(city == "San Jose") {
        $.getJSON("http://52.39.148.231:3000/getData?filename=stationTripCluster3DSanJose", function(data) {
            getTripData(data);
            console.log(tripsData);
            plotChordGraph();
        });
    } else if(city == "San Francisco") {
        $.getJSON("http://52.39.148.231:3000/getData?filename=stationTripCluster3DSanFrancisco", function(data) {
            getTripData(data);
            console.log(tripsData);
            plotChordGraph();
        });
    } else if(city == "Palo Alto") {
        $.getJSON("http://52.39.148.231:3000/getData?filename=stationTripCluster3DPaloAlto", function(data) {
            getTripData(data);
            console.log(tripsData);
            plotChordGraph();
        });
    } else if(city == "Redwood City") {
        $.getJSON("http://52.39.148.231:3000/getData?filename=stationTripCluster3DRedwood", function(data) {
            getTripData(data);
            console.log(tripsData);
            plotChordGraph();
        });
    } else if(city == "Mountain View"){
        $.getJSON("http://52.39.148.231:3000/getData?filename=stationTripCluster3DMountainView", function(data) {
            getTripData(data);
            console.log(tripsData);
            plotChordGraph();
        });
...
function plotChordGraph() {
    ...
    var w = 1280,
        ...
        r1 = h / 2,
        r0 = r1 - 80;
    /* media queries for screen width less than 1440 */ 
    
     if(window.screen.width <1400){
     //  alert ("media quesries exeution");
        var w = 750,
        h = 500,
        r1 = h / 2,
        r0 = r1 - 80;
